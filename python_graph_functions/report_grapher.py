...
def load_and_process_data():
    ...
    # Calculate real frequency from Real_Required_Clock_Period_ns
    df['Real_Frequency_MHz'] = 1000 / df['Real_Required_Clock_Period_ns']
    
    return df

def create_frequency_comparison_graphs(df):
    """Create graphs comparing real frequency vs expected frequency"""
    ...
    for idx, op_bitwidth in enumerate(op_bitwidth_pairs):
        ...
        ax = axes_flat[idx]
        ax.scatter(subset['Frequency_MHz'], subset['Real_Frequency_MHz'], 
                  alpha=0.7, s=60, color='green')
        
        # Connect the points with lines
        if len(subset) > 1:
            ax.plot(subset['Frequency_MHz'], subset['Real_Frequency_MHz'], 
                   "b--", alpha=0.8, linewidth=2)
        
        # Add diagonal line for reference (expected = real)
        min_freq = min(subset['Frequency_MHz'].min(), subset['Real_Frequency_MHz'].min())
        max_freq = max(subset['Frequency_MHz'].max(), subset['Real_Frequency_MHz'].max())
        ax.plot([min_freq, max_freq], [min_freq, max_freq], 'r-', alpha=0.5, linewidth=1, label='Expected = Real')
        
        ax.set_xlabel('Expected Frequency (MHz)')
        ax.set_ylabel('Real Frequency (MHz)')
    ...
    plt.tight_layout()
    plt.suptitle('Real Frequency vs Expected Frequency by Operator and Bitwidth', 
                 fontsize=14, y=1.02)
    ...
    print(f"Saved frequency comparison graph to: {output_dir / 'frequency_comparison.png'}")
...
def create_individual_hw_resource_graphs(df, resource_type, resource_column, graph_type='full'):
    """Create graphs for a specific hardware resource type with different presentation levels"""
    # Get unique operator-bitwidth combinations
    op_bitwidth_pairs = df['Op_Bitwidth'].unique()
    ...
    for idx, op_bitwidth in enumerate(op_bitwidth_pairs_filtered):
        ...
        if graph_type == 'axes_only':
            # Just axes and grid, no data
            pass
        elif graph_type == 'curve_only':
            # Scatter plot and curve, no color coding
            ax.scatter(subset[resource_column], 
            ...
            for _, row in subset.iterrows():
                ...
                ax.annotate(f'{freq_mhz:.0f}MHz', 
                           ...
                           xytext=(5, 5), textcoords='offset points', 
                           fontsize=8, alpha=0.7)
        else:  # graph_type == 'full'
            # Full graph with latency color coding
            ...
            # Add latency labels instead of frequency
            for _, row in subset.iterrows():
                ax.annotate(f'{int(row["max_latency"])} cyc', 
                           (row[resource_column], row['Real_Required_Clock_Period_ns']),
                           xytext=(5, 5), textcoords='offset points', 
                           fontsize=8, alpha=0.7)
    ...
    # Different titles for different graph types
    if graph_type == 'axes_only':
        title = f'{resource_type} vs Clock Period Template'
    elif graph_type == 'curve_only':
        title = f'{resource_type} vs Clock Period (Frequency Annotations)'
    else:
        title = f'{resource_type} vs Clock Period (Latency Colored & Annotated)'
    ...
    if graph_type == 'axes_only':
        filename = f'{resource_type.lower()}_vs_clock_period_template.png'
    elif graph_type == 'curve_only':
        filename = f'{resource_type.lower()}_vs_clock_period_freq.png'
    else:
        filename = f'{resource_type.lower()}_vs_clock_period_latency.png'
    
    plt.savefig(output_dir / filename, dpi=300, bbox_inches='tight')
    print(f"Saved {resource_type} {graph_type} graph to: {output_dir / filename}")

def create_all_hw_resource_graphs(df):
    """Create separate graphs for each hardware resource type in all three presentation formats"""
    ...
    graph_types = ['axes_only', 'curve_only', 'full']
    
    for resource_name, column_name in resource_types.items():
        for graph_type in graph_types:
            print(f"Creating {resource_name} vs clock period graphs ({graph_type})...")
            create_individual_hw_resource_graphs(df, resource_name, column_name, graph_type)
...
def main():
    ...
    print("Creating frequency comparison graphs...")
    create_frequency_comparison_graphs(df)
